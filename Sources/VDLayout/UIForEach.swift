...
public struct UIForEach<Data: Collection, ID: Hashable>: UI {
	...
	public let element: (Data.Element) -> UILayout
	public let getID: (Data.Element) -> ID
	
	public var layout: UILayout {
		UILayout(flat: data.map {
			element($0).id(getID($0))
		})
	}
	
	public init(_ data: Data, id: @escaping (Data.Element) -> ID, @UIBuilder element: @escaping (Data.Element) -> UILayout) {
		self.data = data
...
extension UIForEach where Data.Element: Identifiable, Data.Element.ID == ID {
	...
	public init(_ data: Data, @UIBuilder element: @escaping (Data.Element) -> UILayout) {
		self.data = data
		self.element = element
		self.getID = { $0.id }
	}
